...
class SheetsAPI {
     ...
     * a specific google spreadsheet based on an id given.
     * 
    ...
    /**
     * Grabs an instance of the sheetsAPI class through the sheetsAPIInstance
     * variable
     * 
     * @throws IOException              If the credentials.json file cannot be found
     *                                  by the getCredentials function
    ...
    String createSpreadsheet(String title) throws IOException {
        ...
        spreadsheet = sheetsServiceHandler.spreadsheets().create(spreadsheet).setFields("spreadsheetId").execute();
        return spreadsheet.getSpreadsheetId();
    }

    /**
     * Returns the objects contained in within each cell of the listed spreadsheet
     * in the listed range.
     * 
     * @param spreadsheetId The unique ID of the wanted spreadsheet. Can be found
     *                      within the url of the spreadsheet
    ...
    public List<List<Object>> getValues(String spreadsheetID, String range) throws IOException {
        ValueRange response = sheetsServiceHandler.spreadsheets().values().get(spreadsheetID, range).execute();
     ...
     * @param valueOption   option decribing the type of values being written
     *                      (formulas or text)
     * @param value         the values being written into the speadsheet
     * @throws IOException If speadsheet does not exists
     */
    public void writeToSpreadsheet(String spreadsheetID, String range, String valueOption, List<List<Object>> value)
            throws IOException {
        ValueRange newValues = new ValueRange().setValues(value);
        UpdateValuesResponse result = sheetsServiceHandler.spreadsheets().values()
                .update(spreadsheetID, range, newValues).setValueInputOption(valueOption).execute();
    ...
    public void renameTab(String spreadsheetID, int tabNumber, String newTitle) throws IOException {
        ...
        if (tabNumber >= 0 && tabNumber < sheets.size()) {
            ...
            System.out.println(String.format("Tab Number %d does not exist", tabNumber));
    }
     ...
     * final String range = "POs!A1:AX3"; SheetsAPI sheet =
     * SheetsAPI.getSheetsAPI(); List<List<Object>> values =
     * sheet.getValues(spreadsheetId, range); if (values == null ||
     * values.isEmpty()) { System.out.println("No data found."); } else {
     * //System.out.println("Name, Major"); int count = 0; for (List row : values) {
     * // Print columns A and E, which correspond to indices 0 and 4.
     * //System.out.printf("%s, %s\n", row.get(0), row.get(4));
     * //row.ensureCapacity(26+23); for(int i = 0; i < row.size(); i++)
     * System.out.printf("%s   ", row.get(i));
     * 
     * count++; System.out.printf("Size: %d\n", row.size()); System.out.println(); }
     * 
     * System.out.println("update spreadsheet"); //sheet.writeToSpreadsheet(range,
     * "RAW", values); } }
     */
}